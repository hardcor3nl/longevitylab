      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <AnimatedSection className="flex items-end justify-between mb-10">
          <div>
            ...
            <h2 className="font-display text-4xl text-ink">Top-Ranked Products</h2>
            <p className="text-muted mt-2 max-w-lg">Ranked by evidence quality, safety, and mechanistic plausibility across {products.length} longevity interventions.</p>
          </div>
            ...
            <AnimatedSection key={p.id} delay={i * 0.08}>
              <Link href="/database"
                ...
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span className="font-display text-2xl text-ink">{p.evidenceScore}</span>
                </div>
                <h3 className="font-display text-lg text-ink group-hover:text-green transition-colors mb-1">{p.name}</h3>
                <p className={`font-mono text-[10px] uppercase tracking-widest mb-2 ${evidenceColor[p.evidenceLevel]}`}>{p.evidenceLevel} Evidence</p>
